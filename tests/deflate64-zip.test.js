...
await TestUtils.test("Read and parse deflate64 ZIP file", async () => {
	...
	for (const file of files) {
		...
		console.log(
			`   Compression method: ${file.compressionMethod} ${file.compressionMethod === 9 ? "(deflate64)" : ""}`,
		);
		...
		if (file.compressionMethod === 9) { // Deflate64
			...
			try {
				...
				if (calculatedCRC === file.crc32) {
					...
					throw new Error(
						`CRC-32 mismatch: calculated 0x${calculatedCRC.toString(16).toUpperCase()}, expected 0x${
							file.crc32.toString(16).toUpperCase()
						}`,
					);
				}
